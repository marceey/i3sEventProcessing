...
class Reduction():
    ...
    def getFullscaleCoord(self, x, y):
        return x * self.div, min((x + 1) * self.div, self.width_fullscale), y * self.div, min((y + 1) * self.div,
                                                                                              self.height_fullscale)
...
class EventCount(Reduction):
    ...
    def __init__(self, sim_time=-1, input_ev=np.zeros((0, 4)), coord_t=3, div=2, width=-1, height=-1, threshold=1,
                 plot=False):
        """
        Arguments:
        - events (numpy array): events to be reduced
        ...
        self.input = self.input[self.input[:,
                                self.coord_t].argsort()]  # this method needs the events to be sorted according to the timestamps
...
class LogLuminance(Reduction):
        ...
        for n_pixel in tqdm(range(self.width_downscale * self.height_downscale)):
            ...
            for fullscale_w in range(w_min, w_max):
                for fullscale_h in range(h_min, h_max):
                    ev_pixel = self.input[
                        np.logical_and(self.input[:, 0] == fullscale_w, self.input[:, 1] == fullscale_h)]
    ...
    def extractIntersections(self, timestamps, y_curve):
        ...
        for y_ev in y_levels:
            t_ev = [np.interp(y_ev, y_seg, t_seg) if y_seg[0] < y_seg[-1] else np.interp(y_ev, y_seg[::-1], t_seg[::-1])
                    for y_seg, t_seg in zip(y_segments, t_segments) if min(y_seg) <= y_ev <= max(y_seg)]
            T_events += t_ev
